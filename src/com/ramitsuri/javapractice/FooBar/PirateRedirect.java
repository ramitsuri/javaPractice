...
/**
 * Created by ramit on 12/12/2015.
 */
...
public class PirateRedirect {
    public static int answer(int[] numbers) {

        int i = numbers[0];
        List<Integer> piratesVisited = new ArrayList<>();
        boolean isLoop = false;
        piratesVisited.add(i);
        int firstVisitedIndex = 0;
        while(!isLoop){
            if(piratesVisited.contains(numbers[i])){
                firstVisitedIndex = piratesVisited.indexOf(numbers[i]);
                isLoop = true;

            }
            else{
                piratesVisited.add(numbers[i]);
                i = numbers[i];
            }
        }

        for (int j = 0; j<piratesVisited.size();j++){
            System.out.println(piratesVisited.get(j));
        }
        return piratesVisited.size() - firstVisitedIndex;
...
public class PirateRedirect {
    public static int answer(int[] numbers) {
        ...
        int i = 0;
        int[] piratesVisited = new int[5000];
        int visitedIndex = 0;
        boolean isLoop = false;
        boolean contains = false;
        piratesVisited[0] = numbers[0];
        visitedIndex = visitedIndex + 1;
        i=numbers[0];
        while(!isLoop) {
            for (int j = 0; j < visitedIndex; j++) {
                if (piratesVisited[j] == numbers[i]) {
                    contains = true;
                    isLoop = true;
                }
            }
            if (contains == false) {
                ...
                piratesVisited[visitedIndex] = numbers[i];
                visitedIndex = visitedIndex + 1;
                i = numbers[i];
            }
        ...
        for (int j = 0; j<visitedIndex;j++) {
            System.out.println(piratesVisited[j]);
        }
        return visitedIndex;
